    ...
    <div className='solution-container mt-10 gap-28 sm:w-full sm:flex-col lg:flex lg:w-4/5 lg:flex-row'>
      <div>
        <div className='number-container flex h-10 w-10 items-center justify-center rounded-full p-2'>
          <div className='desc-text text-lg text-gray-300'>0{props.id}</div>
        </div>
      </div>

      <div className='flex flex-col gap-6'>
        <div className='info-text solution-title text-white'>{props.title}</div>

        <div className='desc-text text-lg text-gray-300'>{props.desc}</div>

        <div className='wrapper flex items-center'>
          <Link href={props.url} className='info-text solution-link'>
            Read More
          </Link>
